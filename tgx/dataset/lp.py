...
class LPDatasetLoader(TemporalDatasetLoader):
    ...
    def load(self) -> Tuple[
        ...
        train_start_time, val_time, test_time = list(np.quantile(timestamps, [train_start, train_end, val_end]))
        ...
        test_node_set = set(sources[test_time_mask]).union(set(destinations[test_time_mask]))

        # Sample new test nodes
        new_test_node_set = set(self.rng.sample(
            list(test_node_set),
            int(0.1 * n_total_unique_nodes)
        ))

        # Create masks for new test nodes
        new_test_source_mask = np.array([src in new_test_node_set for src in sources])
        new_test_destination_mask = np.array([dst in new_test_node_set for dst in destinations])

        # Mask for observed edges (no new test nodes involved)
        observed_edges_mask = np.logical_and(~new_test_source_mask, ~new_test_destination_mask)

        # Training mask (before val time, no new nodes)
        train_mask = np.logical_and.reduce([train_start_time < timestamps, timestamps <= val_time, observed_edges_mask])
        ...
        assert len(seen_node_set & new_test_node_set) == 0, "Overlap between seen and new test nodes (unseen)"
        ...
        if not self.different_new_nodes_between_val_and_test:
            new_old_node_val_data = Data(
                sources[new_old_node_val_mask], destinations[new_old_node_val_mask],
                timestamps[new_old_node_val_mask], edge_idxs[new_old_node_val_mask],
                labels[new_old_node_val_mask]
            )
            new_old_node_test_data = Data(
                sources[new_old_node_test_mask], destinations[new_old_node_test_mask],
            ...
            new_new_node_val_data = Data(
                sources[new_new_node_val_mask], destinations[new_new_node_val_mask],
                timestamps[new_new_node_val_mask], edge_idxs[new_new_node_val_mask],
                labels[new_new_node_val_mask]
            )
            new_new_node_test_data = Data(
                sources[new_new_node_test_mask], destinations[new_new_node_test_mask],
            ...
            new_old_node_val_data = new_old_node_test_data = new_new_node_val_data = new_new_node_test_data = None

        unseen_nodes_num = len(new_test_node_set)

        return (
            node_features, edge_features, full_data, train_data, val_data, test_data,
            new_node_val_data, new_node_test_data, new_old_node_val_data, new_old_node_test_data,
            new_new_node_val_data, new_new_node_test_data, new_test_node_set
        )