...
class LCDatasetLoader(TemporalDatasetLoader):
    ...
    def load(
        ...
        train_start_time, val_time, test_time = list(
            np.quantile(graph_df.ts, [train_start, train_end, val_end])
        )
        ...
        full_data = Data(
            ...
            edge_idxs,
            labels,
            edge_features if edge_features is not None else None,
        )

        # Create train/val/test masks based on time
        if self.use_validation:
            train_mask = np.logical_and(
                train_start_time < timestamps, timestamps <= val_time
            )
            val_mask = np.logical_and(timestamps <= test_time, timestamps > val_time)
        else:
            train_mask = np.logical_and(
                train_start_time < timestamps, timestamps <= test_time
            )
    ...
    def get_train_val_test_nodes(
        ...
        all_nodes = np.unique(
            np.concatenate([full_data.sources, full_data.destinations])
        )
