...
class TemporalModel(LightningModule, ABC):
    ...
    def training_step(self, batch: Data, batch_idx: int) -> torch.Tensor:
        ...
        model_outputs = self.evaluator.postprocess_outputs(model_outputs, batch=batch, stage="train")
        ...
        self.evaluator.log_metrics(metrics, "train", self)
    ...
    def validation_step(self, batch: Data, batch_idx: int) -> torch.Tensor:
        ...
        model_outputs = self(batch)
        model_outputs = self.evaluator.postprocess_outputs(model_outputs, batch=batch, stage="val")
        targets = self.evaluator.get_targets(batch, model_outputs, "val")

        # Get node information for inductive evaluation
        node_info = getattr(batch, 'node_info', None)
        ...
        self.evaluator.log_metrics(metrics, "val", self)
    ...
    def test_step(self, batch: Data, batch_idx: int) -> torch.Tensor:
        ...
        model_outputs = self(batch)
        model_outputs = self.evaluator.postprocess_outputs(model_outputs, batch=batch, stage="test")
        ...
        self.evaluator.log_metrics(metrics, "test", self)
